...
from compressai.layers import CSSM, HyperAnalysis, HyperSynthesis, subpel_conv3x3, Global_inter_Context, Channel_wise_Context
...
class VMIC(nn.Module):
    ...
    def __init__(self, N=192, M=320):
        ...
        self.channel_context = nn.ModuleList(
            Channel_wise_Context(dim=slice_ch * i, out_dim=slice_ch*2) if i else None
            for i in range(slice_num)
        )

        self.inter_context = nn.ModuleList(
            Global_inter_Context(dim=slice_ch * i, out_dim=slice_ch) if i else None
            for i in range(slice_num)
        )
    ...
    def forward(self, x):
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                inter_ctx = self.inter_context[idx](torch.cat(y_hat_slices, dim=1))
                # Anchor(Use channel context and hyper params)
                params_anchor = self.entropy_parameters_anchor[idx](torch.cat([channel_ctx, inter_ctx, hyper_params], dim=1))
                scales_anchor, means_anchor = params_anchor.chunk(2, 1)
                # split means and scales of anchor
                scales_anchor = ckbd_anchor(scales_anchor)
                means_anchor = ckbd_anchor(means_anchor)
                ...
                params_nonanchor = self.entropy_parameters_nonanchor[idx](torch.cat([channel_ctx, inter_ctx, hyper_params], dim=1))
                scales_nonanchor, means_nonanchor = params_nonanchor.chunk(2, 1)
                # split means and scales of nonanchor
                scales_nonanchor = ckbd_nonanchor(scales_nonanchor)
                means_nonanchor = ckbd_nonanchor(means_nonanchor)
